...
private:
    ...
    const typename StepperNewmarkExplicitAFormAppAction<Scalar>::ACTION_LOCATION actLoc)
  {
    ...
    switch(actLoc) {
      ...
      case StepperNewmarkExplicitAFormAppAction<Scalar>::BEFORE_SOLVE:
      {
        modType = X_BEFORE_SOLVE;
        x = workingState->getX();
        break;
      }
      case StepperNewmarkExplicitAFormAppAction<Scalar>::AFTER_SOLVE:
      {
...
public:
  ...
  enum MODIFIER_TYPE {
    ...
    X_BEFORE_SOLVE,   ///< Modify \f$x\f$ before the solve.
    X_AFTER_SOLVE,    ///< Modify \f$x\f$ after the solve.
